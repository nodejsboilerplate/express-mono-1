...
import { users, userContacts, userPhones, userEmails, userAddresses, userProfiles } from "@/database";
...
async function createUser(overrides: Partial<any> = {}) {
  ...
  if (!id) throw new Error("Fixture setup failed: createUserCore returned empty string");
  return id;
}
...
async function createContact(userId: string, overrides: Partial<any> = {}) {
  const id = await userService.createUserContact({ user_id: userId, socials: [], ...overrides }, pgDb);
  if (!id) throw new Error("Fixture setup failed: createUserContact returned empty string");
  return id;
}
...
describe("UserService.createUserCore", () => {
  ...
  test("rejects a duplicate username at the database level", async () => {
    ...
    await expect(
      userService.createUserCore(validUserPayload({ username: payload.username }), pgDb)
    ).rejects.toThrow();
  });
});
...
describe("UserService.createUserProfile", () => {
  test("creates a profile for a user", async () => {
    ...
    const [row] = await pgDb.select().from(userProfiles).where(eq(userProfiles.id, profileId));
...
describe("UserService.createUserContact / createUserPhone / createUserEmail", () => {
  test("creates a contact for a user with no id supplied", async () => {
    ...
    const contactId = await userService.createUserContact({ user_id: userId, socials: [] }, pgDb);

    expect(typeof contactId).toBe("string");
    expect(contactId).not.toBe("");

    const [row] = await pgDb.select().from(userContacts).where(eq(userContacts.id, contactId));
    expect(row?.user_id).toBe(userId);
  });
  ...
  test("creates a phone tied to a contact", async () => {
    ...
    const phoneId = await userService.createUserPhone(
      { user_id: userId, contact_id: contactId, phone_code: "+1", phone: "5551234567" },
      pgDb
    );
    ...
    const [row] = await pgDb.select().from(userPhones).where(eq(userPhones.id, phoneId));
    expect(row?.user_id).toBe(userId);
  ...
  test("creates an email tied to a contact", async () => {
    ...
    const emailId = await userService.createUserEmail(
      { user_id: userId, contact_id: contactId, email: `contact-${crypto.randomUUID()}@example.com` },
      pgDb
    );
  ...
  test("throws when phone_code is missing (DB NOT NULL, no service-level validation)", async () => {
    ...
    await expect(
      userService.createUserPhone(
        { user_id: userId, contact_id: contactId, phone: "5551234567" } as any,
        pgDb
      )
    ).rejects.toThrow();
  });
...
describe("UserService.createUserAddress", () => {
  test("creates an address for a user", async () => {
    ...
    const addressId = await userService.createUserAddress(validAddressPayload(userId), pgDb);

    expect(typeof addressId).toBe("string");
    expect(addressId).not.toBe("");

    const [row] = await pgDb.select().from(userAddresses).where(eq(userAddresses.id, addressId));
    expect(row?.user_id).toBe(userId);
...
describe("UserService.updateUserCore", () => {
  test("updates a user's username", async () => {
    ...
    const result = await userService.updateUserCore({ id: userId, username: newUsername }, pgDb);
  ...
  test("returns an empty string when the user doesn't exist", async () => {
    const result = await userService.updateUserCore({ id: crypto.randomUUID(), username: "ghost" }, pgDb);
    expect(result).toBe("");
  });
});
...
describe("UserService.updateUserProfile", () => {
  ...
  test("updates a profile's first_name", async () => {
    const userId = await createUser();
    const profileId = await userService.createUserProfile({ user_id: userId, first_name: "Old" }, pgDb);
...
describe("UserService.updateUserPhone / updateUserEmail / updateUserAddress", () => {
  ...
  test("updates an email scoped to its user", async () => {
    ...
    const contactId = await createContact(userId);
    const emailId = await userService.createUserEmail(
      { user_id: userId, contact_id: contactId, email: `old-${crypto.randomUUID()}@example.com` },
      pgDb
    );

    const newEmail = `new-${crypto.randomUUID()}@example.com`;
    const result = await userService.updateUserEmail({ id: emailId, user_id: userId, email: newEmail }, pgDb);
    expect(result).toBe(emailId);

    const [row] = await pgDb.select().from(userEmails).where(eq(userEmails.id, emailId));
...
describe("UserService.updateUserContact", () => {
  ...
  test("updates a contact's socials", async () => {
    ...
    const result = await userService.updateUserContact(
      { user_id: userId, socials: [{ type: Socials.FACEBOOK, url: "https://facebook.com/mahin" }] },
      pgDb
    );
    expect(result).not.toBe("");

    const [row] = await pgDb.select().from(userContacts).where(eq(userContacts.user_id, userId));
    expect(row?.socials).toEqual([{ type: "facebook", url: "https://facebook.com/mahin" }]);
  });

  test("returns an empty string when no contact row exists", async () => {
    const userId = await createUser(); // no contact created

    const result = await userService.updateUserContact({ user_id: userId, socials: [] }, pgDb);
    expect(result).toBe("");
  });
});
...
describe("UserService.deleteUserContact / deleteUserPhone / deleteUserEmail / deleteUserAddress", () => {
  test("deletes a contact scoped to its user", async () => {
    ...
    const result = await userService.deleteUserContact({ id: contactId, user_id: userId }, pgDb);
    expect(result).toBe(contactId);

    const rows = await pgDb.select().from(userContacts).where(eq(userContacts.id, contactId));
  ...
  test("returns an empty string when deleting a contact belonging to another user", async () => {
    ...
    const result = await userService.deleteUserContact({ id: contactId, user_id: otherUserId }, pgDb);
    expect(result).toBe("");
  ...
  test("deletes a phone scoped to its user", async () => {
    ...
    const result = await userService.deleteUserPhone({ id: phoneId, user_id: userId }, pgDb);
    expect(result).toBe(phoneId);
  });
  ...
  test("deletes an email scoped to its user", async () => {
    ...
    const result = await userService.deleteUserEmail({ id: emailId, user_id: userId }, pgDb);
    expect(result).toBe(emailId);
  });
  ...
  test("deletes an address scoped to its user", async () => {
    ...
    const result = await userService.deleteUserAddress({ id: addressId, user_id: userId }, pgDb);
    expect(result).toBe(addressId);
  });
});
...
describe("UserService.deleteUser", () => {
  ...
  test("cascades and deletes the user's profile, phone, and address", async () => {
    const userId = await createUser();
    await userService.createUserProfile({ user_id: userId, first_name: "Mahin" }, pgDb);
    const contactId = await createContact(userId);
    const phoneId = await userService.createUserPhone(
      { user_id: userId, contact_id: contactId, phone_code: "+1", phone: "5551234567" },
      pgDb
    ...
    const [profileRow] = await pgDb.select().from(userProfiles).where(eq(userProfiles.user_id, userId));
    const [phoneRow] = await pgDb.select().from(userPhones).where(eq(userPhones.id, phoneId));
    const [addressRow] = await pgDb.select().from(userAddresses).where(eq(userAddresses.id, addressId));
