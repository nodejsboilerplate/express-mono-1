...
export class UserZValidation {
    ...
    static profile = z4.object({
        first_name: z4.string({ error: "First name must be a string" }).trim().max(100, { error: "First name must be at most 100 characters" }).nullish(),
        last_name: z4.string({ error: "Last name must be a string" }).trim().max(100, { error: "Last name must be at most 100 characters" }).nullish(),
        avatar: z4.url({ error: "Invalid avatar URL" }).nullish(),
        cover_img: z4.url({ error: "Invalid cover image URL" }).nullish(),
        nickname: z4.string({ error: "Nickname must be a string" }).trim().max(100, { error: "Nickname must be at most 100 characters" }).nullish(),
    ...
    static contact = z4.object({
        socials: z4.array(this.socialLink, { error: "Socials must be an array" }).default([]),
    });
    ...
    static phone = z4.object({
        ...
        is_primary: z4.boolean({ error: "is_primary must be a boolean" }).optional(),
        phone_code: z4
    ...
    static address = z4.object({
        addr_name: z4.string({ error: "Address name must be a string" }).trim().max(100, { error: "Address name must be at most 100 characters" }).optional(),
        addr_line_1: z4
        ...
        addr_line_2: z4.string({ error: "Address line 2 must be a string" }).trim().max(255, { error: "Address line 2 must be at most 255 characters" }).optional(),
        city: z4
            .string({ error: "City is required" })
            .trim()
            .min(1, { error: "City is required" })
            .max(100, { error: "City must be at most 100 characters" }),
        state: z4.string({ error: "State must be a string" }).trim().max(100, { error: "State must be at most 100 characters" }).optional(),
        post_code: z4.string({ error: "Post code must be a string" }).trim().max(20, { error: "Post code must be at most 20 characters" }).optional(),
        country: z4
